...
{
    // Known verse counts per surah (114 surahs)
    const VERSE_COUNTS = [
    ...
    public function run(): void
    {
        ...
        foreach (self::VERSE_COUNTS as $surahId => $count) {
            for ($v = 1; $v <= $count; $v++) {
                $verses[] = [
                    'id'            => $verseId,
                    'surah_id'      => $surahId,
                    'verse_number'  => $v,
                    'text_ar'       => '',  // Placeholder — actual text not needed for page mapping
                    'text_id'       => null,
                    'created_at'    => now(),
                    'updated_at'    => now(),
                ];
            ...
            // Batch insert per surah to avoid memory issues
            if (count($verses) >= 500) {
                DB::table('quran_verses')->insert($verses);
                $verses = [];
            }
        }
